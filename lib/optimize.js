...
window.optimize = (function () {
    ...
    optimize.fmin = function (func, x0, opts) {
        ...
        fsim = [];
        fsim[0] = func(x0);
        //对于数组的每一维都进行处理生成一个新的数组并计算器函数值，然后储存到解空间数组中
        for (i = 0; i < N; i++) {
            y = vector.copy(x0);
        ...
        while (iterations < maxiter) {
            ...
            xbar = vector.fmult(1.0 / N, vector.reduce(sim.slice(0, sim.length - 1)));
    ...
    optimize.ParticleSwarmOptimization = function(objectiveFunction, x0, numParticles, options) {
        ...
        options.c1 = _q(options.c1) ? options.c1 : 0.05;
        options.c2 = _q(options.c2) ? options.c2 : 0.01;
        options.velocityLimit = _q(options.velocityLimit) ? options.velocityLimit : 0.10;
        options.maxIterations = _q(options.maxIterations) ? options.maxIterations : 50 * numDimensions;
        // 初始化粒子群
        let particles = [];
        ...
        for (let i = 0; i < numParticles; i++) {
            ...
            for (let j = 0; j < numDimensions; j++) {
                particle.position[j] *= window.randomNormal();  // 初始位置随机化
                particle.velocity[j] = 0;  // 初始速度为零
            }

            particle.personalBestPosition = particle.position.slice();  // 初始个体最优位置为初始位置
            particle.personalBestValue = objectiveFunction(particle.position);  // 计算初始个体最优值

            // 更新全局最优位置和值
            if (particle.personalBestValue < globalBestValue) {
                globalBestPosition = particle.personalBestPosition.slice();
        ...
        const updateParticles = () => {
            for (let i = 0; i < numParticles; i++) {
                ...
                for (let j = 0; j < numDimensions; j++) {
                    // 更新速度
                    particle.velocity[j] = options.inertiaWeight * particle.velocity[j] +
                        options.c1 * Math.random() * (particle.personalBestPosition[j] - particle.position[j]) +
                        options.c2 * Math.random() * (globalBestPosition[j] - particle.position[j]);

                    // 限制速度范围（可选）
                    if (options.velocityLimit) {
                        particle.velocity[j] = Math.max(Math.min(particle.velocity[j], options.velocityLimit), -options.velocityLimit);
                    }
                    ...
                    // 限制位置范围（可选）
                    if (options.positionLimit) {
                        particle.position[j] = Math.max(Math.min(particle.position[j], options.positionLimit), -options.positionLimit);
                    }
                }
