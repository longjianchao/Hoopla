  ...
  Image = (function(_super) {
    ...
    Image.prototype._getFrameAsync = function(buffers, callback, opts) {
      ...
      worker.onmessage = function(e) {
        ...
        // Ensure we don't write beyond the pixels array
        var end = Math.min(start + arr.length, pixels.length);
        var writeLength = end - start;
        if (writeLength > 0) {
          pixels.set(arr.subarray(0, writeLength), start);
        }
        start = end;
        i += 1;
        if (i === _this.nBuffers || start >= pixels.length) {
