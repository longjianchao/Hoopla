...
(function(exports) {
	...
	Lens.prototype.ang2pix = function(ang){
		...
		return { x: (ang.x / this.pixscale + this.w/2).toFixed(2), y: (this.h/2 - ang.y / this.pixscale).toFixed(2) }
	}
	//----------------------------------------------------------------------------
	...
	Lens.prototype.calculateImage = function(){
		...
		for(row = 0 ; row < this.h ; row++){
			for(col = 0 ; col < this.w ; col++){
				...
				let src = this.source[0];
				if (src.type === "mge") {
					for (let g = 0; g < src.gaussians.length; g++) {
						const gauss = src.gaussians[g];
						const q = gauss.q; // 确保 0 < q ≤ 1
						const phi_rad = -gauss.ang * Math.PI / 180.0; // 根据实际定义调整正负
						const cos_phi = Math.cos(phi_rad);
						const sin_phi = Math.sin(phi_rad);
						// 坐标变换（显式声明变量）
						const xnew = d.x * cos_phi + d.y * sin_phi;
						const ynew = d.y * cos_phi - d.x * sin_phi;
						// 计算椭圆半径
						const sigma_sq = gauss.sigma * gauss.sigma;
						const r2 = (xnew * xnew) / sigma_sq + (ynew * ynew) / (sigma_sq * q * q);
						// 添加归一化因子
						const normalization = 1 / (2 * Math.PI * gauss.sigma * gauss.sigma * q);
						v += gauss.I * normalization * Math.exp(-0.5 * r2);
					}
				} else {
